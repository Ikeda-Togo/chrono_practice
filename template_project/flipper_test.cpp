...
//    mstator = chrono_types::make_shared<ChBodyEasyBox>(1.3, 1.3, 0.1, 3000, material, collision_type);
//    mstator->SetCollide(false);
//    mstator->SetPos(mpos + ChVector<>(0, 2, -0.25));
//    mstator->SetRot(Q_from_AngAxis(CH_C_PI_2, VECT_Z));
...
//
//    mrotor = chrono_types::make_shared<ChBodyEasyBox>(0.1, 2, 0.1, 10000, material, collision_type);
//    mrotor->SetPos(mstator->GetPos()+ ChVector<>(0, 1, -0.15));
//    msystem.Add(mrotor);
//
...
public:
    MyEventReceiver(ChIrrAppInterface* myapp, MySimpleTank* atank, double* angle) {
        ...
        scrollbar_flipper->setMax(100);
        scrollbar_flipper->setPos(50);
        text_flipper =
            application->GetIGUIEnvironment()->addStaticText(L"Flipper ", rect<s32>(650, 70, 750, 85), false);
    }
...
private:
    ...
    IGUIStaticText* text_flipper;
    IGUIScrollBar* scrollbar_flipper;
};
...
int main(int argc, char* argv[]) {
    ...
    ChVector<> positionA2(-3, 3, -2);
    std::shared_ptr<ChBody> stator2;
    std::shared_ptr<ChBody> rotor2;
    //CreateStatorRotor(stator2, rotor2, material, my_system, mytank->wheelRB->GetPos());
    CreateStatorRotor(stator2, rotor2, material, my_system, positionA2);

    GetLog() << "clower pos: " << mytank->truss->GetPos() << "\n\n";
    GetLog() << "clower pos: " << mytank->wheelRB->GetPos() << "\n\n";
    ...
    //link1->Initialize(stator2, mytank->truss, ChCoordsys<>(stator2->GetPos(), QUNIT));
    //link1->Lock(true);
    ...
    MyEventReceiver receiver(&application, mytank, &angle);
