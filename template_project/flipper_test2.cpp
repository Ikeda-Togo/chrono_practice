...
int main(int argc, char* argv[]) {
    ...
    rotmotor2->Initialize(rotor2,                // body A (slave)
        ...
        ChFrame<>(stator2->GetPos())  // motor frame, in abs. coords
    );
    ...
    auto acc = chrono_types::make_shared<ChAccelerometerSensor>(mytank->truss,    // body to which the IMU is attached
    ...
    auto gyro = chrono_types::make_shared<ChGyroscopeSensor>(mytank->truss,     // body to which the IMU is attached
        imu_update_rate,    // update rate
        imu_offset_pose,    // offset pose from body
    ...
    auto mag = chrono_types::make_shared<ChMagnetometerSensor>(mytank->truss,            // body to which the IMU is attached
        imu_update_rate,  // update rate
        imu_offset_pose,  // offset pose from body
