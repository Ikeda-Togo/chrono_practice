...
public:
    ...
    MySimpleTank(ChSystemNSC& my_system,        ///< the Chrono physical system
        ...
        double shoelength = 0.042; //ベルトユニットの長さ
        double shoe2length = 0.028; //ベルトユニットの長さ
        double shoemass = 1;       //ベルトユニットの重さ
        double radiustrack = 0.052; //クローラの半径
        double wheeldiameter = 0.047 * 2; //プーリーの直径 初期0.045
        ...
        ChVector<> cyl_displA(0, 0.05, 0); //円柱の中心の座標
        ChVector<> cyl_displB(0, -0.05, 0);
        double cyl_hthickness = 0.02; //0.03だと高さ0.06の円柱になる
...
public:
    ...
    MySimpleFlipper(ChSystemNSC& my_system,        ///< the Chrono physical system
        ...
        double wheeldiameter = 0.045 * 2; //タイヤ直径
        int nwrap = 6;
        int ntiles = 6;
        double rlwidth = 0.216;
